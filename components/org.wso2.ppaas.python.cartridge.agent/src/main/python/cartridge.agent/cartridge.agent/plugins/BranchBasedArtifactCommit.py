...
class BranchBasedArtifactCommit(IArtifactCommitPlugin):
    """
    Default implementation for the artifact checkout handling
    """

    def __init__(self):
        super(BranchBasedArtifactCommit, self).__init__()
    ...
    def commit(self, repo_info):
        ...
        if errors.strip() == "":
            commit_hash = AgentGitHandler.find_between(output, "[" + git_repo.branch, "]").strip()
