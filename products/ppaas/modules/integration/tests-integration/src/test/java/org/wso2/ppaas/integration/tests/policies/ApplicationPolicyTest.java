 ...
 */
public class ApplicationPolicyTest extends PPaaSIntegrationTest {
    ...
    public ApplicationPolicyTest() throws Exception {
    }

    @Test(description = "Deploy application policy", timeOut = APPLICATION_TEST_TIMEOUT)
    public void testApplicationPolicy() throws Exception {
        ...
        assertEquals(bean.getAlgorithm(), "one-after-another",
                String.format("The expected algorithm %s is not found in %s",
                        "one-after-another", applicationPolicyId));
        assertEquals(bean.getId(), applicationPolicyId, String.format("The expected id %s is not found",
                applicationPolicyId));
        ...
        assertEquals("network-partition-application-policy-test-1", bean.getNetworkPartitions()[0],
        ...
        for (PropertyBean propertyBean : bean.getProperties()) {
            if (propertyBean.getName().equals("networkPartitionGroups")) {
                assertEquals(propertyBean.getValue(), "network-partition-application-policy-test-1," +
    ...
    @Test(description = "Deploy application policy 2", timeOut = APPLICATION_TEST_TIMEOUT)
