...
public class ADCBranchPerTenantTestCase extends PythonAgentIntegrationTest {
    private static final Log log = LogFactory.getLog(ADCBranchPerTenantTestCase.class);
    private static final int ADC_TIMEOUT = 300000;
    private static final String APPLICATION_PATH = "/tmp/ADCBranchPerTenantTestCase";
    ...
    public ADCBranchPerTenantTestCase() throws IOException {
    }
    ...
    public void setupADCTest() throws Exception {
        log.info("Setting up ADCBranchPerTenant test case");
...
//        }
        Files.createDirectories(tempWorkDir.toPath());
        ...
        // configure Python agent with configurations provided in resource path
        super.setupWithoutStarting();

        // remove DefaultCheckoutArtifact yapsy files
        File defaultArtifactCheckoutYapsyFile = new File(agentPath + File.separator +
                "plugins" + File.separator + "DefaultArtifactCheckout.yapsy-plugin");
        FileUtils.forceDelete(defaultArtifactCheckoutYapsyFile);
        File defaultArtifactCommitYapsyFile = new File(agentPath + File.separator +
                "plugins" + File.separator + "DefaultArtifactCommit.yapsy-plugin");
        FileUtils.forceDelete(defaultArtifactCommitYapsyFile);

        // start Python agent
        super.start(ADC_TIMEOUT);
    ...
    @Test(timeOut = ADC_TIMEOUT, groups = { "all" })
    public void testADC() throws Exception {
                        ...
                        for (String line : newLines) {
                            ...
                            if (repoCloned && !fileCreated) {
                                try {
                                    if (!file.createNewFile()) {
                                        ...
                                        log.info("File " + file.getPath() + " created successfully");
                                    }
                                    // write to file
                                    String msg = "test data";
                                    Files.write(Paths.get(file.getAbsolutePath()), msg.getBytes());
                                    // wait till file get committed
                                    try {
                                        Thread.sleep(10000);
                            ...
                            if (fileCreated && line.contains("ArtifactUpdateTask end of iteration") && !fileDeleted) {
                                if (!file.delete()) {
                                    ...
                                    log.info("File " + file.getPath() + " deleted successfully");
                                }
                                // wait till file is deleted and pushed
                                try {
                                    Thread.sleep(10000);
                                } catch (InterruptedException ignored) {}
        ...
        log.info("ADC Branch per Tenant Test completed");
    }
