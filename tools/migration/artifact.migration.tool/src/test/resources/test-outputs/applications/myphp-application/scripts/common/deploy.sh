...
curl -X POST -H "Content-Type: application/json" -d "@${artifacts_path}/myphp-application.json" -k -v -u ${var_username}:${var_password} ${var_base_url}api/applications
sleep 1
...
curl -X POST -H "Content-Type: application/json" -d "@${artifacts_path}/domain-mapping.json" -k -u ${var_username}:${var_password} ${var_base_url}api/applications/myphp-application/domainMappings
echo "Adding domain mappings..."
