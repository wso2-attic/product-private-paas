...
define appserver::deploy ($security, $target, $owner, $group, $agent_home) {
  ...
  file { "/tmp/plugins":
    ...
    recurse      => true,
    source       => [
      'puppet:///modules/appserver/plugins/']
  }
  ...
  exec {
    ...
    "Remove_${name}_temporory_modules_directory":
      ...
      "Copy_${name}_modules_to_agent_home":
      path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/java/bin/',
      command => "cp -r /tmp/plugins/* ${agent_home}/plugins; chown -R ${owner}:${owner} ${agent_home}/plugins; chmod -R 755 ${agent_home}/plugins",
      require => File["/tmp/plugins"];

    "Remove_/tmp/plugins_temporory_modules_directory":
      path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/java/bin/',
      command => "rm -rf /tmp/plugins",
      require => Exec["Copy_${name}_modules_to_agent_home"];
  }
}
