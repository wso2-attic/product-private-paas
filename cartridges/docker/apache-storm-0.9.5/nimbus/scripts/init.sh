...
if [ "${START_CMD}" = "PCA" ]; then
    ...
    echo "Configuring Apache Storm UI..."
    echo "Environment variables:"
    printenv
    pushd ${CONFIGURATOR_HOME}
    python configurator.py
    popd
    echo "APACHE STORM NIMBUS configured successfully"

    echo "Starting APACHE STORM NIMBUS..."
    ${CARBON_HOME}/bin/storm nimbus
    echo "APACHE STORM NIMBUS started successfully"
fi
