...
startUI() {
	...
	container_id=`docker run -e STORM_TYPE=ui -e ZOOKEEPER_HOSTNAME=${zookeeper_member_ip} -e NIMBUS_HOSTNAME=${nimbus_member_ip} -d -P --name ${name} apache/storm-supervisor:0.9.5`
