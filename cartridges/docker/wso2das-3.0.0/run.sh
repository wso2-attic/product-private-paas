...
startWkaMember () {
    ...
    container_id=`docker run -e CONFIG_PARAM_ZK_HOST=10.100.7.80 -e CONFIG_PARAM_PROFILE=$1 -e CONFIG_PARAM_CLUSTERING=true -d -P --name ${name} wso2/das:3.0.0`
...
startMember() {
	name="wso2das-$1-${memberId}"
    container_id=`docker run -e CONFIG_PARAM_ZK_HOST=10.100.7.80 -e CONFIG_PARAM_CLUSTERING=true -e CONFIG_PARAM_WKA_MEMBERS="[${wka_member_ip}:4100]" -e CONFIG_PARAM_PROFILE=$1 -d -P --name ${name} wso2/das:3.0.0`
